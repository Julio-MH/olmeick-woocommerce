...
# Écrire le script PHP de génération dans un fichier temporaire
cat > /tmp/genkeys.php <<'GENKEYS_PHP'
<?php
...
$key_id = $db->insert_id;
if ($key_id > 0) {
    echo json_encode(['consumer_key' => $ck, 'consumer_secret' => $cs, 'key_id' => (int)$key_id]);
} else {
    echo json_encode(['error' => $db->error]);
}
GENKEYS_PHP

KEY_RESULT=$(php /tmp/genkeys.php 2>&1)
...
RewriteRule ^generate-keys\.php$ - [L]
