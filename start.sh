...
# Générer les clés via wp eval (charge WP + WC nativement)
KEY_RESULT=$(wp eval '
$ck = "ck_" . bin2hex(random_bytes(20));
$cs = "cs_" . bin2hex(random_bytes(20));
$ck_hashed = hash_hmac("sha256", $ck, "wc-api");
global $wpdb;
$wpdb->delete($wpdb->prefix . "woocommerce_api_keys", array(), array());
...
echo json_encode(array("consumer_key" => $ck, "consumer_secret" => $cs, "key_id" => (int)$key_id));
' --path="$WP_DIR" --allow-root 2>&1)
